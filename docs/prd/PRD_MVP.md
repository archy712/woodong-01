# PRD — 우동(Woodong) 계모임/동호회 모임 관리 앱

- **문서 버전**: v1.5
- **작성일**: 2026-08-23
- **작성자**: archy712 (archy712@gmail.com)
- **레포지토리(코드네임)**: `moim-ops`
- **대상 서비스명(사용자 노출)**: 우동 (Woodong)
- **기반 프롬프트**: `docs/prd/PRD_MVP_PROMPT.md`
- **변경 이력**:
  - v1.0 (2026-08-23): 최초 작성
  - v1.1 (2026-08-23): prd-validator 기술 검증 결과 반영. Supabase 프로젝트 공용 사용 원칙 확정(5.0절), 데이터 모델에 `woodong_` 접두어 적용 및 스키마 보완, Naver OAuth 미지원 사실 정정 및 대안 제시(3.6, 9장), 계정 자동 연결 정책 재작성(3.6), 공지사항 테이블 추가, RLS 재귀 회피/Storage 정책 보완(4.2), MVP 범위 재조정(7장), 리스크 항목 보강(9장)
  - v1.2 (2026-08-23): `docs/roadmap/ROADMAP_MVP.md`와의 교차 검토 과정에서 발견된 불일치 수정 — `woodong_groups.cover_image_url`(text, URL)이 4.2절의 "모임 대표 이미지 버킷도 비공개"라는 Storage 정책과 모순되던 것을 `woodong_expenses.receipt_object_path`와 동일한 패턴인 `cover_image_object_path`(오브젝트 경로 + 조회 시 서명 URL 발급)로 정정(5.0, 5.2절)
  - v1.3 (2026-08-24): Task 015 진행 중 발견된 제약 반영 — 이 프로젝트가 공유하는 Supabase 무료 플랜은 커스텀 SMTP 미설정 시 이메일 템플릿 수정이 불가능하고 기본 메일 서버가 프로젝트 팀 멤버 외 주소로는 발송되지 않아, 임의의 신규 가입자에게 확인 메일을 보낼 수 없음(Supabase 공식 문서로 확인). 이에 따라 **회원가입 이메일 확인 절차를 1차 MVP 범위에서 제외**하고 가입 즉시 세션이 발급되도록 재설계(3.6절 AC 수정). 커스텀 SMTP(예: Resend) 연동 후 재도입 여부는 별도 결정 필요
  - v1.4 (2026-08-24): 같은 이메일 발송 제약으로 **비밀번호 찾기(로그아웃 상태의 이메일 재설정 링크) 플로우도 1차 MVP 범위에서 제외**. 대신 로그인된 사용자가 마이페이지에서 현재 비밀번호 확인 후 새 비밀번호로 변경하는 "화면 내 비밀번호 변경"으로 대체(계정 탈취 방지를 위해 이메일 소유 확인 없는 로그아웃 상태 재설정은 의도적으로 제공하지 않음). 로그인 자체가 불가능한 완전한 계정 잠김 상황은 1차 MVP에서 자기 서비스 복구 수단이 없으며 관리자 수동 대응 필요(9장 리스크 항목 추가). `app/auth/confirm/route.ts`(호출부 없음), `app/auth/forgot-password`, `app/auth/update-password`는 삭제, 3.6·6.1절 AC 수정
  - v1.5 (2026-08-24): 알림 매체 로드맵 재검토 — 공유 Supabase 무료 SMTP 제약(v1.3)으로 이메일 채널의 실효성이 낮아진 점, 카카오톡 알림톡의 높은 도입 난이도(비즈니스 계정·템플릿 사전 검수·발송대행사 계약)와 건당 발송 비용, 동호회 일반 사용자층에 Slack이 맞지 않는 점을 종합해 **카카오톡 알림톡/Slack/이메일을 알림 채널 로드맵에서 완전히 제외**하고, 구현 난이도와 사용자 진입장벽이 모두 낮은 **웹 푸시(Web Push, PWA 기반)**를 2차 확장의 외부 알림 채널로 채택. 1차 MVP(앱 내 알림)는 변경 없음. `woodong_notification_preferences`/`woodong_notifications`의 `channel` CHECK 제약을 `('web_push','in_app')`로 축소(2장, 3.3, 4.2~4.4, 5.13, 7.2, 9장 수정)

---

## 1. 서비스 개요

| 항목           | 내용                                                                                                                               |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| 서비스명       | 우동 (Woodong) — "우리동호회"의 줄임말이자 친숙한 음식 이름의 언어유희                                                             |
| 한 줄 정의     | 동호회/모임의 운영, 회비 정산, 투표를 한 곳에서 관리하는 모바일 우선 웹 서비스                                                     |
| 포지셔닝       | "계모임"보다 "동호회 활동·소속감"을 우선 연상시키는 브랜딩. 딱딱한 관리툴이 아니라 친근한 동호회 친구                              |
| 타겟 사용자    | 동호회/러닝크루/등산모임 운영진(총무), 계모임 총무, 소규모 커뮤니티(10~50명) 멤버                                                  |
| 핵심 문제      | 엑셀 + 카카오톡 단톡방으로 회비·정산·투표를 관리하며 발생하는 비효율: 수기 정산, 미납 파악 어려움, 공지 누락, 투표 취합의 번거로움 |
| 핵심 가치 제안 | 총무의 반복 업무(회비 독촉, 정산 집계, 투표 취합, 공지 발송)를 자동화해 "총무 1인 부담"을 줄인다                                   |

### 1.1 브랜드/네이밍 가이드

- **톤앤매너**: 딱딱한 관리툴이 아닌 "친근하고 재미있는 동호회 친구" 느낌 유지. 오류 메시지, 빈 상태(empty state), 알림 문구까지 일관된 캐릭터 톤 사용
- **카피 예시**: "우동, 모임을 부탁해", "오늘도 우동이 정산해드릴게요"
- **로고 컨셉**: 우동 그릇 모티프 — 그릇 = 모임이 열리는 "자리", 그릇을 둘러싼 사람 아이콘으로 "함께 모이는" 이미지 표현
- **컬러**: 코럴/앰버 계열(따뜻함, 활기) — 기존 스타터킷의 `tailwind.config.ts` / `app/globals.css`의 HSL CSS 변수(`--primary` 등)를 이 컬러 계열로 교체 예정 (v3 방식 하이브리드 구성 유지)
- **도메인 후보**: `woodong.app` / `woodong.kr` / `woodong.co.kr`(1순위), `woodong.com` · `uridonghoe.com`(브랜드 보호용) — 등록 전 whois.kr, 키프리스(kipris.or.kr) 상표 검색으로 최종 확인 필요 (개발 착수와 무관한 별도 트랙, 마케팅/법무 담당 확인 필요)

### 1.2 프로젝트 관리 정보

| 항목          | 내용                                                                             |
| ------------- | -------------------------------------------------------------------------------- |
| 내부 코드네임 | `moim-ops` (레포지토리명, 내부 문서 전용)                                        |
| 사용자 노출명 | 우동 (Woodong) — 코드네임과 분리 운영, 코드/변수명은 영문 코드네임 기준으로 작성 |

---

## 2. 기술 스택 (고정 — 변경 제안 불필요)

본 프로젝트는 기존 Next.js 16 + Supabase Auth 스타터킷(`woodong-01`) 위에 구축한다. 아래 스택은 프롬프트 지침과 현재 코드베이스(`CLAUDE.md`) 기준을 그대로 따른다.

| 영역                | 기술                                                                                                                                 | 비고                                                                                                                                                                                                                                                                                                                               |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Frontend            | Next.js 16 (App Router)                                                                                                              | `proxy.ts`(구 middleware), `cacheComponents: true`(Cache Components) 사용 중. request-time API(`cookies()`, `headers()`, `params`, `searchParams`)는 비동기이며 해당 컴포넌트는 `<Suspense>` 경계 필수                                                                                                                             |
| Language            | TypeScript 5.x                                                                                                                       | strict 모드                                                                                                                                                                                                                                                                                                                        |
| Styling             | Tailwind CSS v4 + shadcn/ui (`new-york` 스타일)                                                                                      | `components.json` 기준, `cn()`(`lib/utils.ts`)으로 클래스 병합                                                                                                                                                                                                                                                                     |
| Icons               | lucide-react                                                                                                                         | 아이콘 갤러리(`components/icons/icon-categories.ts`)와 연동                                                                                                                                                                                                                                                                        |
| Backend/DB/Auth     | Supabase (PostgreSQL, Auth, Storage, Edge Functions)                                                                                 | RLS 필수 적용                                                                                                                                                                                                                                                                                                                      |
| Supabase 클라이언트 | `lib/supabase/client.ts`(브라우저) / `lib/supabase/server.ts`(서버, 요청마다 신규 생성) / `lib/supabase/proxy.ts`(`updateSession()`) | 세션 확인은 `getUser()`가 아닌 `getClaims()` 사용                                                                                                                                                                                                                                                                                  |
| 인증                | 이메일/비밀번호 + Google OAuth(네이티브) + Kakao OAuth(네이티브)                                                                     | **Naver는 Supabase Auth 네이티브 미지원(기술 검증으로 확인됨)** — 1차 MVP 범위에서 제외, 대안은 3.6.1절 참고                                                                                                                                                                                                                       |
| 알림 연동           | 웹 푸시(Web Push, PWA 기반 — VAPID + Service Worker), 앱 내 알림(종 모양 아이콘 + 알림센터)                                          | 1차 MVP는 앱 내 알림만 구현. 웹 푸시는 2차 확장(7장, 9장 참고) 대상이며, 카카오톡 알림톡/Slack/이메일은 구현 난이도(대행사 계약·템플릿 심사·발송 비용) 대비 실효성이 낮아 로드맵에서 완전히 제외했다(v1.5). 정기 발송(회비 리마인드 등)은 Supabase Cron(pg_cron) 기반 스케줄러로 2차 확장 — Edge Function 자체에는 스케줄러가 없음 |
| Supabase 프로젝트   | 기존 계약된 무료 플랜 프로젝트 1개를 다른 앱과 **공유**(신규 프로젝트 생성 없음)                                                     | 상세 원칙은 5.0절 참고. 우동 전용 테이블은 전부 `woodong_` 접두어로 격리하고 기존 테이블은 ALTER/DROP 금지                                                                                                                                                                                                                         |
| 다국어              | 기존 `lib/i18n/`(쿠키 + `Accept-Language`, ko/en/ja/zh) 구조 재사용                                                                  | 신규 문자열은 `Dictionary` 타입과 4개 언어 파일에 모두 추가                                                                                                                                                                                                                                                                        |
| 배포/개발           | Claude Code 기반 개발, Vercel 배포                                                                                                   | 기존 스타터킷 배포 파이프라인 재사용                                                                                                                                                                                                                                                                                               |

---

## 3. 핵심 기능 요구사항

각 기능은 User Story(`~로서, ~하고 싶다, 왜냐하면 ~이기 때문이다`)와 Given/When/Then 수용 기준으로 정의한다.

### 3.1 모임 메인 페이지 (랜딩)

**설명**: 서비스 소개, 사용 기술 스택 소개(인포그래픽), 로그인 전/후 다른 CTA를 노출하는 공개 랜딩 페이지. 기존 `app/page.tsx`, `app/about/page.tsx` 패턴(얇은 `Page` + `<Suspense>` + `XxxContent`)을 재사용한다.

**User Story**

- 비로그인 방문자로서, 서비스가 무엇을 해결해주는지 한눈에 이해하고 싶다. 왜냐하면 가입 여부를 빠르게 판단해야 하기 때문이다.
- 로그인한 사용자로서, 메인 페이지에서 바로 내 모임 목록으로 이동하고 싶다. 왜냐하면 매번 로그인 상태를 재확인하고 싶지 않기 때문이다.

**수용 기준**

- Given 비로그인 사용자가 `/`에 접속했을 때, When 페이지가 로딩되면, Then 서비스 소개 섹션, 핵심 기능 3가지 요약, 기술 스택 인포그래픽, "무료로 시작하기"(회원가입 유도) CTA가 노출된다.
- Given 로그인한 사용자가 `/`에 접속했을 때, When 페이지가 로딩되면, Then CTA가 "내 모임 보기"로 대체되고 클릭 시 모임 목록 페이지로 이동한다.
- Given `getClaims()` 등 request-time API를 사용하는 서버 컴포넌트가 포함될 때, When 페이지를 렌더링하면, Then 해당 컴포넌트는 `<Suspense fallback={...}>`로 감싸져 있어 `cacheComponents: true` 환경에서 blocking-route 에러가 발생하지 않는다.

---

### 3.2 모임 관리 (CRUD)

**설명**: 모임 생성/수정/삭제, 멤버 초대(링크 또는 코드), 역할 구분(총무/일반회원).

**User Story**

- 로그인한 사용자로서, 새 모임을 생성하고 싶다. 왜냐하면 우리 동호회의 회비·투표를 관리할 공간이 필요하기 때문이다.
- 총무로서, 모임 정보(이름, 설명, 대표 이미지, 월 회비 기본값)를 수정하고 싶다. 왜냐하면 모임 운영 중 정보가 바뀔 수 있기 때문이다.
- 총무로서, 초대 링크 또는 초대 코드를 발급하고 싶다. 왜냐하면 카카오톡 등으로 멤버를 쉽게 초대해야 하기 때문이다.
- 신규 참여자로서, 초대 링크나 코드를 입력해 모임에 가입하고 싶다. 왜냐하면 별도 승인 절차 없이 빠르게 합류하고 싶기 때문이다.
- 총무로서, 멤버의 역할(총무/일반회원)을 변경하거나 멤버를 모임에서 제외하고 싶다. 왜냐하면 운영진 위임이나 탈퇴 처리가 필요하기 때문이다.
- 총무로서, 모임을 삭제하고 싶다. 왜냐하면 더 이상 운영하지 않는 모임을 정리해야 하기 때문이다.

**수용 기준**

- Given 로그인한 사용자가 모임 생성 페이지에서 이름/설명/월 회비 기본값을 입력했을 때, When "모임 만들기"를 클릭하면, Then 새 모임이 생성되고 생성자가 자동으로 `총무` 역할로 등록되며 모임 상세 페이지로 이동한다.
- Given 필수 항목(모임 이름)이 비어 있을 때, When "모임 만들기"를 클릭하면, Then 유효성 검사 에러가 표시되고 생성 요청이 전송되지 않는다.
- Given 총무가 모임 상세 페이지에서 "멤버 초대"를 클릭했을 때, When 초대 링크 생성을 요청하면, Then 만료 기간이 설정된 고유 초대 코드/링크가 생성되어 복사할 수 있다.
- Given 신규 참여자가 유효한 초대 링크로 접속했을 때, When 로그인 상태에서 "참여하기"를 클릭하면, Then `woodong_group_members`에 `일반회원`으로 등록되고 모임 상세 페이지로 이동한다.
- Given 신규 참여자가 만료되었거나 잘못된 초대 코드를 입력했을 때, When "참여하기"를 클릭하면, Then "유효하지 않은 초대 코드입니다" 에러가 표시된다.
- Given 이미 해당 모임의 멤버인 사용자가 같은 모임의 초대 링크로 재접속했을 때, When "참여하기"를 클릭하면, Then `woodong_group_members`의 `UNIQUE(group_id, user_id)` 제약에 의해 중복 멤버십이 생성되지 않고 모임 상세로 바로 이동한다.
- Given 총무가 기존 초대 코드를 재발급했을 때, When 새 코드가 생성되면, Then 이전 `woodong_group_invites` 레코드는 `is_active = false`(`revoked_at` 기록)로 전환되어 더 이상 참여에 사용할 수 없다.
- Given 일반회원이 다른 회원의 역할을 변경하려고 시도할 때, When 요청이 전송되면, Then 권한 없음(RLS 차단) 응답으로 거부된다.
- Given 모임에 총무(admin)가 1명만 남아 있을 때, When 해당 총무가 자신의 역할을 일반회원으로 변경하거나 모임을 탈퇴하려고 시도하면, Then "마지막 총무는 역할을 변경하거나 탈퇴할 수 없습니다. 먼저 다른 멤버를 총무로 지정해주세요" 에러가 표시되고 요청이 거부된다(총무 단일 실패점 방지, 9장 참고).
- Given 총무가 모임 삭제를 요청했을 때, When 확인 다이얼로그에서 "삭제"를 재확인하면, Then 모임과 연관된 회비/정산/투표 데이터가 함께 삭제(또는 소프트 삭제)되고 모임 목록 페이지로 이동한다.

---

### 3.3 모임 알림

**설명**: 공지사항(`woodong_announcements`) 등록 → 사용자별 알림 채널 설정(웹 푸시/앱내)에 따라 발송. 알림 유형: 공지, 회비 납부 리마인드, 투표 시작/마감, 정산 리포트 발행. **1차 MVP는 앱 내 알림만 구현**하며, 정기 발송이 필요한 리마인드류는 실시간 스케줄러 없이 "조회 시점 지연 처리(lazy)"로 대체한다(스케줄러 인프라 결정은 9장 참고).

**User Story**

- 총무로서, 공지사항을 작성해 모든 멤버에게 알리고 싶다. 왜냐하면 단톡방 공지가 묻히는 문제를 해결해야 하기 때문이다.
- 일반회원으로서, 내가 원하는 채널(웹 푸시/앱내)로만 알림을 받고 싶다. 왜냐하면 채널별로 알림 피로도가 다르기 때문이다.
- 일반회원으로서, 앱 내 알림센터에서 지난 알림 이력을 확인하고 읽음 처리하고 싶다. 왜냐하면 놓친 공지를 다시 찾아보고 무엇을 확인했는지 구분하고 싶기 때문이다.

**수용 기준**

- Given 총무가 공지사항 작성 후 "발송"을 클릭했을 때, When 발송 요청이 처리되면, Then `woodong_announcements`에 공지 레코드가 생성되고, 각 멤버의 `woodong_notification_preferences`에서 `in_app`이 활성화된 채널 기준으로 `woodong_notifications`에 알림이 기록되어 앱 내 알림센터에 노출된다(1차 MVP는 in_app만 발송, 외부 채널은 2차).
- Given 멤버가 회비 미납 상태일 때, When 해당 멤버가 회비 대시보드 또는 알림센터에 진입하면(1차: 실시간 스케줄러 대신 조회 시점 lazy 처리), Then 마지막 리마인드 발송 이후 설정된 주기가 지났는지 확인해 필요 시 새 리마인드 알림이 기록된다. 실제 배치 스케줄러(pg_cron) 기반 자동 발송은 2차 확장에서 적용한다.
- Given 투표 마감일시가 지난 상태로 멤버가 해당 투표 상세/목록을 조회할 때(1차: lazy 마감), When 페이지가 로딩되면, Then 서버에서 마감 처리 및 결과 집계가 수행되고 조회한 멤버에게 결과가 표시되며 `woodong_notifications`에 결과 알림이 기록된다. 참여 여부와 무관하게 전원에게 실시간으로 알림을 발송하는 것은 2차 확장(pg_cron 스케줄러) 대상이다.
- Given 정산 리포트가 발행되었을 때, When 발행 액션이 완료되면, Then 모임 전체 멤버에게 "정산 리포트 발행" 알림이 `woodong_notifications`에 기록된다(정산 발행 기능 자체는 2차 확장, 7장 참고).
- Given 사용자가 마이페이지에서 특정 채널(예: 웹 푸시)을 비활성화했을 때, When 새 알림 이벤트가 발생하면, Then 해당 채널로는 발송되지 않고 활성화된 다른 채널(예: 앱내)로만 발송된다.
- Given 외부 알림 채널(웹 푸시, 2차 확장) 발송이 실패했을 때, When 재시도 정책이 트리거되면, Then 지정된 횟수만큼 재시도 후 최종 실패 시 앱 내 알림으로 폴백 발송되고 `woodong_notifications` 테이블에 실패 상태가 기록된다.
- Given 사용자가 알림센터에서 특정 알림 항목을 클릭했을 때, When 상세로 이동하면, Then `woodong_notifications.read_at`/`clicked_at`이 갱신되고 알림센터 목록에서 읽음/안읽음이 구분되어 표시된다.

---

### 3.4 회비 관리

**설명**: 회원별 납부 현황 대시보드, 미납자 리마인드, 지출 내역 등록, 정산 리포트. **공수 비중이 커서 3.4-a(회비 현황 — 1차 MVP)와 3.4-b(지출/정산 — 2차 확장)로 분리한다(7장 참고).** 회비 "항목/사이클"(`woodong_due_cycles`)과 멤버별 "청구"(`woodong_dues`)를 분리해, 항목 단위 수정/리마인드 주기 설정이 가능하도록 설계를 보완했다(기존 PRD의 단일 `dues` 테이블 설계 오류 수정).

**User Story**

- 총무로서, 이번 달 회비를 누가 냈고 누가 안 냈는지 한눈에 보고 싶다. 왜냐하면 매번 엑셀로 대조하는 것이 번거롭기 때문이다.
- 총무로서, 특정 멤버의 납부 상태를 수동으로 "납부완료/부분납부/미납"으로 변경하고 싶다. 왜냐하면 계좌이체 확인은 수동으로 대사해야 하기 때문이다.
- 총무로서, 미납자에게 설정된 주기로 리마인드 알림을 보내고 싶다. 왜냐하면 매번 직접 독촉 메시지를 보내기 번거롭기 때문이다.
- 총무로서(2차), 지출 내역(카테고리, 금액, 담당자, 영수증)을 등록하고 싶다. 왜냐하면 회비 사용 내역을 투명하게 기록해야 하기 때문이다.
- 총무 또는 일반회원으로서(2차), 기간별 수입/지출/잔액이 정리된 정산 리포트를 확인하고 다운로드하고 싶다. 왜냐하면 모임 재정 상태를 투명하게 공유해야 하기 때문이다.

**수용 기준 — 3.4-a 회비 현황 (1차 MVP)**

- Given 총무가 회비 항목(제목/대상 기간/금액/리마인드 주기)을 생성했을 때, When 생성이 완료되면, Then `woodong_due_cycles` 레코드가 1건 생성되고, 해당 시점의 모든 활성 멤버에 대해 `woodong_dues` 레코드가 "미납" 상태로 자동 생성된다(멤버당 `woodong_dues` 1건, `UNIQUE(due_cycle_id, user_id)`로 중복 생성 방지).
- Given 회비 대시보드에 진입했을 때, When 페이지가 로딩되면, Then 멤버별 납부/미납/부분납부 상태가 진행률 바 형태(인포그래픽 스타일)로 표시되고 전체 납부율이 상단에 요약된다.
- Given 총무가 특정 멤버의 상태를 "납부완료"로 변경했을 때, When 저장하면, Then `woodong_payments`에 납부 이력이 기록되고, `woodong_dues.status`는 해당 `due_id`에 연결된 `woodong_payments.amount` 합계와 `woodong_dues.amount`를 비교해 `paid`/`partial`로 자동 갱신되며 대시보드가 즉시 반영된다.
- Given 총무가 미납자 리마인드 주기(예: 3일마다)를 `woodong_due_cycles.reminder_interval_days`에 설정했을 때, When 멤버가 회비 대시보드 또는 알림센터를 조회하면(1차: lazy 처리), Then 마지막 리마인드 이후 주기가 지난 미납 멤버에 한해 리마인드 알림이 `woodong_notifications`에 기록된다. 실시간 배치 발송은 2차(pg_cron)로 확장한다.

**수용 기준 — 3.4-b 지출/정산 (2차 확장)**

- Given 총무가 지출 내역을 등록하며 영수증 이미지를 첨부했을 때, When "등록"을 클릭하면, Then 영수증이 Supabase Storage의 비공개(private) 버킷에 업로드되고 `woodong_expenses` 레코드가 생성되며, 조회 시에는 서명된 URL(`createSignedUrl`)로만 접근 가능하다(4.2절 Storage 정책 참고).
- Given 필수 항목(금액, 카테고리)이 누락된 지출 등록 요청일 때, When "등록"을 클릭하면, Then 유효성 검사 에러가 표시되고 등록되지 않는다.
- Given 총무가 특정 기간(예: 2026년 8월)의 정산 리포트 생성을 요청했을 때, When 생성이 완료되면, Then 해당 기간의 총 수입, 총 지출, 잔액이 `woodong_settlements`에, 항목별 상세 내역이 `woodong_settlement_items`에 발행 시점 스냅샷으로 함께 생성되고 웹 뷰 또는 PDF로 다운로드할 수 있다.
- Given 일반회원이 정산 리포트 페이지에 접근했을 때, When 페이지가 로딩되면, Then 조회는 가능하지만 수정/삭제 버튼은 노출되지 않는다(RLS 및 UI 이중 방어).

> **1차 MVP 범위**: 위 3.4-a만 구현한다. 지출 등록/영수증 첨부/정산 리포트 발행·PDF는 3.4-b로 2차 확장에 포함한다(7장 참고). 단, 회비 대시보드의 "잔액" 표기는 1차에서는 노출하지 않거나 "수입만 집계"로 한정한다(지출 데이터가 없으므로).

---

### 3.5 투표 관리

**설명**: 투표 생성(CRUD), 객관식/찬반, 마감일 설정, 익명/실명 옵션, 마감 시 자동 집계 및 알림.

**User Story**

- 총무로서, 다음 모임 날짜나 장소를 정하기 위한 투표를 만들고 싶다. 왜냐하면 단톡방에서 의견을 취합하는 것이 비효율적이기 때문이다.
- 총무로서, 투표를 객관식(단일 선택) 또는 찬반(2지선다)으로 설정하고, 필요하면 복수 선택도 허용하고 싶다. 왜냐하면 안건 성격에 따라 형식이 다르기 때문이다.
- 총무로서, 투표를 익명 또는 실명으로 설정하고 싶다. 왜냐하면 민감한 안건은 익명이 필요하기 때문이다.
- 일반회원으로서, 마감 전까지 투표에 참여하고 싶다. 왜냐하면 내 의견을 반영하고 싶기 때문이다.
- 모든 멤버로서, 투표가 마감되면 자동으로 집계된 결과를 확인하고 싶다. 왜냐하면 별도로 결과를 취합할 필요가 없어야 하기 때문이다.

**수용 기준**

- Given 총무가 투표 제목, 선택지 2개 이상, 마감일시, 복수 선택 허용 여부(`allow_multiple`), 익명 여부를 입력했을 때, When "투표 만들기"를 클릭하면, Then `woodong_votes`와 `woodong_vote_options` 레코드가 생성되고 멤버 전원에게 "새 투표 시작" 알림이 `woodong_notifications`에 기록된다.
- Given 선택지가 1개 이하이거나 마감일시가 과거일 때, When "투표 만들기"를 클릭하면, Then 유효성 검사 에러가 표시되고 생성되지 않는다.
- Given `allow_multiple = false`인 투표에서 일반회원이 마감 전 투표 상세 페이지에 진입했을 때, When 선택지 1개를 고르고 "투표하기"를 클릭하면, Then `woodong_vote_responses`에 응답이 기록되고 `UNIQUE(vote_id, user_id)` 제약으로 같은 멤버의 중복 투표가 차단된다.
- Given `allow_multiple = true`인 투표일 때, When 멤버가 여러 선택지를 고르고 "투표하기"를 클릭하면, Then 선택한 선택지 수만큼 `woodong_vote_responses`가 생성되되 `UNIQUE(vote_id, user_id, option_id)` 제약으로 동일 선택지 중복 선택만 차단된다.
- Given 투표가 "익명" 옵션으로 설정되었을 때, When 결과를 조회하면, Then `woodong_vote_responses`를 직접 SELECT하는 대신 `SECURITY DEFINER` 집계 함수/뷰를 통해 선택지별 집계 수치만 반환되고 응답자 이름은 노출되지 않는다(4.2절 참고).
- Given 투표가 "실명" 옵션으로 설정되었을 때, When 결과를 조회하면, Then 각 선택지에 투표한 멤버 이름 목록이 함께 표시된다.
- Given 투표 마감일시가 지난 상태에서 멤버가 해당 투표 목록/상세를 조회할 때(1차 MVP: lazy 마감 처리, 실시간 스케줄러는 2차), When 페이지가 로딩되면, Then 서버에서 투표 상태가 "마감"으로 전환되고 결과가 집계되어 조회한 멤버에게 즉시 표시된다.
- Given 총무가 마감 전 투표를 수동으로 조기 마감하려고 할 때, When "지금 마감"을 클릭하면, Then 확인 다이얼로그 이후 즉시 마감 처리와 동일한 로직이 수행된다.
- Given 투표가 (lazy 처리든 수동 마감이든) 방금 마감 상태로 전환되었을 때, When 마감 처리가 완료되면, Then 참여 여부와 무관하게 모임 멤버 전원에게 결과 알림이 `woodong_notifications`에 기록된다(1차: 조회 트리거 기반, 2차: pg_cron 스케줄러로 실시간 발송 전환).

---

### 3.6 인증

**설명**: 이메일 가입/로그인 + Google/Kakao 소셜 로그인(1차 MVP). 소셜 로그인 계정과 이메일 계정의 연결은 Supabase Auth의 플랫폼 기본 동작(verified 이메일 자동 연결)을 그대로 수용하는 정책으로 재설계했다(아래 3.6.2절). 기존 스타터킷의 `app/auth/*`, `components/*-form.tsx`(Client Component에서 `supabase.auth.*` 직접 호출) 패턴을 확장한다.

#### 3.6.1 소셜 로그인 제공자 선정 (기술 검증 반영)

> **정정 사항**: 최초 프롬프트의 "Google/Naver/Kakao 3종"은 기술적으로 그대로 구현할 수 없다. [FACT] Supabase Auth의 네이티브 소셜 provider 목록(Apple, Google, Kakao, Facebook, GitHub 등)에 **Naver는 포함되지 않는다.**

| 제공자   |     Supabase 네이티브 지원     | 1차 MVP 채택 | 비고                                                                                                                                                                                                                                                                                           |
| -------- | :----------------------------: | :----------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Google   |               O                |      ✅      | 이메일이 항상 verified 상태로 제공되어 3.6.2의 자동 연결 정책이 안정적으로 동작                                                                                                                                                                                                                |
| Kakao    |               O                |      ✅      | 국내 사용자 로그인 편의성(카카오 로그인 선호도) 고려 시 필수. **단, Biz App(사업자 정보 등록) 미완료 시 이메일을 제공하지 않을 수 있음** — Supabase 프로젝트에서 "Allow users without an email" 옵션 활성화 필요. 이 경우 해당 사용자는 이메일 기준 자동 연결 대상에서 제외됨(9장 리스크 참고) |
| Naver    | 미지원(네이티브 provider 아님) |      ❌      | Custom OAuth 설정 또는 자체 콜백 구현(Route Handler + `auth.admin` API)이 필요하나, 네이버가 표준 OIDC discovery를 제공하지 않아 적합성이 불확실함. **1차 MVP 범위에서 제외, 2차 확장 착수 전 1~2일 스파이크(PoC)로 구현 가능 여부를 먼저 검증**한다                                           |
| Apple    |          O(네이티브)           |   2차 검토   | 향후 iOS 앱으로 래핑할 경우 App Store 심사 정책상 타 소셜 로그인을 제공하면 Apple 로그인도 필수로 요구됨. 이메일 릴레이(Hide My Email) 대응 필요                                                                                                                                               |
| Facebook |          O(네이티브)           |     보류     | 네이티브 지원은 되나 국내 타겟(동호회/러닝크루) 대비 사용률이 낮아 우선순위 낮음                                                                                                                                                                                                               |

**결론**: 1차 MVP는 **Google + Kakao 2종**으로 축소하고, Naver는 스파이크 결과에 따라 2차 확장 여부를 결정한다. Apple은 iOS 앱 래핑 계획이 구체화되는 시점에 재검토한다.

**User Story**

- 신규 사용자로서, 이메일과 비밀번호로 회원가입하고 싶다. 왜냐하면 별도 소셜 계정 없이도 서비스를 이용하고 싶기 때문이다.
- 신규 사용자로서, Google/Kakao 계정으로 간편하게 로그인하고 싶다. 왜냐하면 새 비밀번호를 만들고 기억하기 번거롭기 때문이다.
- 기존 이메일 가입 사용자로서, 동일한 이메일의 소셜 계정으로 로그인했을 때 내 기존 데이터가 그대로 유지되는지 명확히 알고 싶다. 왜냐하면 계정이 분리되어 모임/회비 데이터에 접근하지 못하게 되는 상황을 원하지 않기 때문이다.
- 로그인한 사용자로서, 마이페이지에서 내 계정에 연결된 로그인 수단 목록을 확인하고 원치 않는 연결을 해제하고 싶다. 왜냐하면 어떤 방법으로 로그인했는지, 계정이 얼마나 노출돼 있는지 파악하고 싶기 때문이다.
- 로그인한 사용자로서, 비밀번호를 잊었을 때 재설정하고 싶다. 왜냐하면 계정에 다시 접근해야 하기 때문이다.

#### 3.6.2 계정 연결 정책 (재작성)

> [FACT] Supabase Auth는 신규 OAuth 로그인의 이메일이 **verified 상태**이고 동일 이메일의 기존 사용자가 있으면, 별도 설정 없이 **자동으로 identity를 연결(auto-linking)**한다. 이 동작을 끄는 공식 옵션은 없다. 따라서 "자동 병합을 막는다"는 기존 정책은 플랫폼 기본 동작과 충돌하여 그대로 구현할 수 없으며, 아래와 같이 **"자동 연결을 수용하고 사후 고지"** 방향으로 재작성한다.
>
> 1. **자동 연결 수용**: 이메일/비밀번호 계정과 OAuth 계정이 동일한 verified 이메일을 가지면 로그인 시점에 자동으로 연결되며, 사용자의 기존 모임/회비/투표 데이터는 그대로 유지된다.
> 2. **사후 고지**: 자동 연결이 발생한 최초 로그인 직후, "기존 [이메일] 계정에 연결되었습니다" 안내 토스트를 노출한다(연결 여부는 `supabase.auth.getUserIdentities()`로 연결 전/후 identity 개수를 비교해 판별).
> 3. **이메일 미제공 예외**: Kakao Biz App 미등록으로 이메일이 없는 계정은 자동 연결 대상이 되지 않고 별도 계정으로 생성된다. 이 경우 마이페이지의 수동 연동 기능을 안내한다.
> 4. **수동 연동/해제**: 마이페이지 "연동된 계정" 섹션에서 `linkIdentity()`(Supabase 프로젝트의 Manual Linking 베타 옵션을 사전 활성화해야 동작)로 추가 연동, `unlinkIdentity()`로 해제할 수 있다. 마지막 남은 identity는 로그인 수단 상실을 막기 위해 해제할 수 없다.
> 5. **2차 확장**: 자동 연결 발생 시 이메일로도 알림을 발송해 사용자가 인지하지 못한 채 연결되는 상황에 대한 방어를 강화한다(1차는 인앱 토스트만).

**수용 기준**

- Given 신규 사용자가 이메일/비밀번호를 입력했을 때, When "회원가입"을 클릭하면, Then Supabase Auth에 계정이 즉시 활성 세션과 함께 생성되고 별도 이메일 확인 절차 없이 보호된 페이지에 접근할 수 있다(1차 MVP는 이메일 확인 절차 제외 — 공유 Supabase 프로젝트가 커스텀 SMTP 없이는 임의 주소로 확인 메일을 보낼 수 없는 제약 때문, v1.3 변경 이력 참고). Supabase 프로젝트의 "Confirm email" 옵션이 꺼져 있어야 한다.
- Given 사용자가 로그인 페이지에서 "Google로 계속하기"를 클릭했을 때, When OAuth 인증이 완료되면, Then Supabase Auth 세션이 생성되고 로그인 전 접근하려던 원래 경로(없으면 모임 목록 페이지)로 리다이렉트된다.
- Given 이메일 `a@example.com`으로 이미 이메일/비밀번호 계정이 존재하고 해당 이메일이 verified 상태일 때, When 동일 이메일의 Kakao 소셜 로그인을 시도하면, Then Supabase Auth 기본 동작에 따라 신규 identity가 기존 계정에 자동으로 연결되고, 로그인 완료 직후 "기존 계정에 연결되었습니다" 안내 토스트가 노출되며 기존 모임/회비 데이터에 그대로 접근할 수 있다.
- Given Kakao Biz App 미등록으로 이메일을 제공받지 못하는 사용자가 Kakao로 로그인할 때, When 인증이 완료되면, Then 이메일 없는 신규 계정으로 가입 처리되고(Supabase 프로젝트의 "Allow users without an email" 옵션 활성화 전제) 마이페이지에서 수동으로 이메일 계정과 연동할 수 있다는 안내가 노출된다.
- Given 로그인한 사용자가 마이페이지 "연동된 계정" 섹션에 진입했을 때, When 페이지가 로딩되면, Then `getUserIdentities()` 기준 현재 연결된 provider 목록이 표시되고, 연결된 provider가 2개 이상일 때만 개별 해제 버튼이 활성화된다(마지막 1개는 해제 불가).
- Given 인증되지 않은 사용자가 `/protected/*` 또는 모임 관련 보호 페이지에 접근했을 때, When `proxy.ts`의 `updateSession()`이 세션을 확인하면, Then 원래 요청 경로를 `next` 쿼리 파라미터로 포함해 `/auth/login`으로 리다이렉트되고, 로그인 성공 후 해당 경로로 복귀한다(현재 `login-form.tsx`의 `/protected` 하드코딩 리다이렉트를 이 흐름으로 대체하는 작업 포함).
- Given 로그인한 사용자가 마이페이지의 "비밀번호 변경" 카드에서 현재 비밀번호와 새 비밀번호를 입력했을 때, When 제출하면, Then 현재 비밀번호가 재인증으로 확인된 뒤 새 비밀번호로 즉시 변경된다(1차 MVP는 이메일 발송 없이 화면 내에서만 처리 — v1.4 변경 이력 참고). 현재 비밀번호가 틀리면 "현재 비밀번호가 올바르지 않아요" 에러가 표시되고 변경되지 않는다. 로그아웃 상태에서 이메일만으로 비밀번호를 재설정하는 기능은 계정 탈취 방지를 위해 1차 MVP에서 제공하지 않는다.

---

### 3.7 디자인 시스템

**설명**: shadcn/ui 컴포넌트 최대 활용, lucide 아이콘 일관 사용, 모바일 우선 반응형(360px~), 데이터 시각화는 인포그래픽 스타일(차트/진행률 바 등).

**User Story**

- 사용자로서, 어떤 화면 크기(360px 스마트폰 ~ 데스크톱)에서도 불편함 없이 서비스를 이용하고 싶다. 왜냐하면 총무는 이동 중 스마트폰으로 업무를 처리하는 경우가 많기 때문이다.
- 사용자로서, 회비 납부율이나 투표 결과를 그래프/진행률 바로 직관적으로 보고 싶다. 왜냐하면 숫자 표만으로는 한눈에 파악하기 어렵기 때문이다.

**수용 기준**

- Given 임의의 페이지를 360px 너비 뷰포트에서 렌더링했을 때, When 페이지를 확인하면, Then 가로 스크롤이 발생하지 않고 모든 인터랙션 요소(버튼, 입력 필드)가 터치 가능한 크기(최소 44x44px)로 표시된다.
- Given 회비 대시보드 또는 투표 결과 페이지가 렌더링될 때, When 데이터가 로딩되면, Then 기존 `/charts` 갤러리에서 검증된 recharts 기반 컴포넌트 패턴을 재사용한 차트/진행률 바로 시각화된다.
- Given 새로운 UI 컴포넌트가 필요할 때, When 컴포넌트를 구현하면, Then 우선 `npx shadcn@latest add`로 shadcn/ui 프리미티브를 사용하고, 레지스트리에 없는 경우에만 `components/` 루트에 직접 구현한다(기존 `date-range-picker.tsx` 등과 동일 패턴).
- Given 아이콘이 필요한 모든 위치에서, When 아이콘을 배치하면, Then lucide-react 아이콘만 사용하고 다른 아이콘 세트를 혼용하지 않는다.

---

### 3.8 푸터 특수 요구사항 (개발자/QA용 내부 문서 페이지)

**설명**: lucide 아이콘 전체 리스트업 페이지(검색 가능), shadcn/ui 전체 컴포넌트 사용 예제 데모 페이지. 이 두 페이지는 개발자/QA용 내부 문서 성격임을 명시. **현재 코드베이스에 이미 `/icons`(아이콘 검색), `/gallery`(shadcn/ui 컴포넌트 데모)가 구현되어 있으므로 신규 개발 없이 재사용하고, 우동 서비스 푸터에 링크만 추가한다.**

**User Story**

- 개발자/QA로서, 서비스 어디서든 아이콘 이름을 검색해 사용 가능한 lucide 아이콘을 확인하고 싶다. 왜냐하면 매번 lucide 공식 사이트를 오가는 것이 비효율적이기 때문이다.
- 개발자/QA로서, shadcn/ui로 만들 수 있는 모든 컴포넌트의 실제 사용 예시를 한 페이지에서 보고 싶다. 왜냐하면 신규 기능 구현 시 이미 존재하는 패턴을 재사용하고 싶기 때문이다.
- 일반 사용자로서, 이 두 페이지가 서비스 핵심 기능이 아님을 명확히 인지하고 싶다. 왜냐하면 실수로 데모 페이지를 실제 기능으로 오해하지 않아야 하기 때문이다.

**수용 기준**

- Given 사용자가 임의의 페이지 하단 푸터를 확인할 때, When 푸터가 렌더링되면, Then "개발자 문서" 그룹 아래 "아이콘 검색"(`/icons`)과 "컴포넌트 갤러리"(`/gallery`) 링크가 시각적으로 구분되어(예: 작은 글씨, 별도 라벨) 노출된다.
- Given 사용자가 `/icons` 페이지에 접속했을 때, When 검색창에 키워드를 입력하면, Then 기존 `categorizeIconName()` 로직(부분 문자열이 아닌 단어 단위 정확 매칭)에 따라 관련 아이콘이 필터링된다(기존 구현 그대로 재사용, 변경 없음).
- Given 사용자가 `/gallery` 페이지에 접속했을 때, When 카테고리 칩을 클릭하면, Then 해당 컴포넌트 섹션으로 앵커 스크롤된다(`GallerySection`의 `id` prop 활용, 기존 구현 그대로 재사용).
- Given 이 두 페이지는, When `proxy.ts`의 `updateSession()` allow-list를 확인하면, Then 로그인 없이 접근 가능한 공개 페이지로 이미 등록되어 있어 추가 라우팅 작업이 필요 없다.

---

## 4. 비기능 요구사항

### 4.1 성능

| 항목               | 목표치                                                                      | 비고                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------ | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 초기 로딩(LCP)     | 모바일 4G(LTE) 환경 기준 2.5초 이내                                         | Next.js 16 Cache Components(`cacheComponents: true`)와 정적 프리렌더링 최대 활용                                                                                                                                                                                                                                                                                           |
| 초기 로딩(LCP)     | 모바일 3G 환경 기준 4초 이내                                                | 회비 대시보드 등 데이터가 많은 페이지는 스켈레톤 UI로 체감 속도 개선                                                                                                                                                                                                                                                                                                       |
| 인터랙션 응답(INP) | 200ms 이내                                                                  | 투표/회비 상태 변경 등 클릭 액션 기준                                                                                                                                                                                                                                                                                                                                      |
| 이미지 최적화      | 모든 사용자 업로드 이미지(영수증, 모임 대표 이미지)는 `next/image`로 최적화 | 원본 업로드 제한: 5MB. [FACT] Supabase Storage의 서버 사이드 이미지 변환(Image Resizing)은 **Pro 플랜 이상**에서만 제공된다(Free 플랜 미지원). 본 프로젝트는 기존 Free 플랜을 공유 중이므로(5.0절), 1차 MVP는 **클라이언트 사이드 리사이즈(Canvas API로 업로드 전 축소)** + `next/image`만으로 대체하고, 플랜 업그레이드 시 서버 사이드 변환으로 전환한다(9장 리스크 참고) |

### 4.2 보안 (Supabase RLS 설계 방향)

- 모든 `woodong_*` 테이블은 기본적으로 RLS(Row Level Security) 활성화(`ENABLE ROW LEVEL SECURITY`)를 원칙으로 한다. **기존 공유 프로젝트의 다른 테이블(`profiles` 포함)에는 RLS 정책이든 스키마든 어떠한 변경도 가하지 않는다**(5.0절 원칙).
- **RLS 무한 재귀 회피(재작성)**: `woodong_group_members`에 해당 `user_id`가 속한 `group_id`의 데이터만 SELECT 가능하도록 정책을 설계하되, `woodong_group_members` 자신의 정책에서 자기 테이블을 서브쿼리로 참조하면 Postgres가 `infinite recursion detected in policy(42P17)` 오류를 낸다(흔한 실패 지점). 이를 피하기 위해 `SECURITY DEFINER` + `search_path` 고정 헬퍼 함수를 먼저 정의하고 모든 정책에서 이 함수를 사용한다.

  ```sql
  create or replace function woodong_is_group_member(p_group_id uuid)
  returns boolean language sql security definer set search_path = '' as $$
    select exists (
      select 1 from public.woodong_group_members
      where group_id = p_group_id and user_id = auth.uid() and status = 'active'
    );
  $$;

  create or replace function woodong_is_group_admin(p_group_id uuid)
  returns boolean language sql security definer set search_path = '' as $$
    select exists (
      select 1 from public.woodong_group_members
      where group_id = p_group_id and user_id = auth.uid() and role = 'admin' and status = 'active'
    );
  $$;
  ```

  `woodong_groups`, `woodong_dues`, `woodong_payments`, `woodong_expenses`, `woodong_settlements`, `woodong_votes`, `woodong_vote_options`, `woodong_vote_responses`, `woodong_announcements`는 SELECT 정책에 `woodong_is_group_member(group_id)`를, 쓰기 정책에 `woodong_is_group_admin(group_id)`를 사용한다. `woodong_group_members` 자신의 SELECT 정책도 동일 함수를 사용해 재귀를 피한다.

- **쓰기 권한 분리**: 회비 상태 변경, 지출 등록, 정산 발행, 투표 생성/조기마감, 멤버 역할 변경, 모임 삭제는 `woodong_is_group_admin(group_id)`가 참인 경우에만 허용한다.
- **투표 응답 보호(정정)**: Postgres RLS는 행 단위 정책만 지원하며 컬럼 단위 차단은 불가능하다("응답자 식별 컬럼만 RLS로 차단"은 구현 불가능한 표현이므로 삭제). 대신 `woodong_vote_responses`는 (a) 본인 레코드만 직접 SELECT/INSERT 가능하도록 행 단위로 제한하고, (b) 익명 투표의 집계 결과는 `SECURITY DEFINER` 함수 또는 뷰로 별도 노출해 응답자 식별 없이 카운트만 반환한다. 중복 투표 방지를 위해 `user_id`는 익명 투표에서도 반드시 저장하되, 이 컬럼은 위 (a)/(b) 경로 밖으로 노출하지 않는다.
- **알림 설정**: `woodong_notification_preferences`는 본인 레코드만 SELECT/UPDATE 가능. `woodong_notifications`는 본인이 수신자인 레코드만 SELECT 가능하고, `read_at`/`clicked_at`만 본인이 UPDATE 가능하도록 컬럼 보호 트리거(기존 프로젝트의 `notifications_protect_columns`와 동일한 패턴을 `woodong_notifications` 전용으로 별도 구현)를 둔다. 모임 전체에 대한 팬아웃 INSERT(공지 발송 등)는 클라이언트 RLS로는 불가능하므로 Edge Function에서 **service_role 키**로 수행한다(서버 전용 환경 변수로 관리).
- **Storage 정책(신규)**: 영수증(`woodong-receipts`)·모임 대표 이미지(`woodong-covers`) 버킷은 모두 **비공개(private)**로 생성한다. 영수증은 재무 정보이므로 public URL로 저장하지 않고, 조회 시점에 `woodong_is_group_member(group_id)`를 만족하는 사용자에게만 `createSignedUrl()`로 임시 URL을 발급한다. `storage.objects`에도 동일한 헬퍼 함수 기반 정책을 적용한다.
- 세션 확인은 코드베이스 관례에 따라 서버 컴포넌트/Route Handler에서 `getClaims()`를 사용하고, 보호 페이지는 `proxy.ts`의 1차 방어 + 개별 서버 컴포넌트의 2차 방어(이중 방어 패턴)를 유지한다.
- 웹 푸시 발송에 사용되는 VAPID 비공개 키와 Supabase `service_role` 키는 서버 전용 환경 변수(`NEXT_PUBLIC_` 접두사 없이)로 관리하고 클라이언트에 노출하지 않는다(VAPID 공개 키는 브라우저 구독 등록에 필요하므로 `NEXT_PUBLIC_` 접두사로 노출한다).

### 4.3 확장성

- 데이터 모델은 `woodong_groups` 테이블에 `type`(예: `동호회`, `계모임`, `스터디`, `취미모임`) 컬럼을 두어 향후 모임 유형이 다양화되어도 스키마 변경 없이 확장 가능하도록 설계한다.
- 알림 채널은 `woodong_notification_preferences.channel`을 `text + CHECK(channel in ('web_push','in_app'))` 제약으로 관리한다(정정: "컬럼 추가만으로 대응"은 표현 오류 — enum 확장은 `ALTER TYPE`, 신규 채널 추가는 `CHECK` 제약 갱신 또는 애플리케이션 레벨 화이트리스트로 대응하는 것이 정확하다. 채널별 목적지(`destination`)는 이미 컬럼으로 존재하므로 신규 채널 추가 시 로직만 추가하면 된다).
- 회비 항목(`woodong_due_cycles`)은 "월 정기 회비"뿐 아니라 향후 "일회성 회비/추가 징수"도 동일 모델로 표현 가능하도록 `due_type`(정기/비정기) 컬럼을 둔다.
- 우동 전용 테이블은 모두 `woodong_` 접두어로 격리되어 있어(5.0절), 향후 우동을 별도 Supabase 프로젝트로 이관해야 할 때도 `woodong_*` 테이블만 `pg_dump`로 추출하면 되어 이관 비용이 낮다.

### 4.4 알림 발송 실패 재시도/폴백 정책

- 외부 채널(웹 푸시) 발송 실패 시 최대 3회, 지수 백오프(1분 → 5분 → 15분)로 재시도한다.
- 최종 실패 시 앱 내 알림(종 아이콘 + 알림센터)으로 반드시 폴백 발송하여 사용자가 서비스 내에서는 정보를 놓치지 않도록 한다.
- 모든 발송 시도(성공/실패/재시도 이력)는 `woodong_notifications` 테이블에 상태(`pending`/`sent`/`failed`/`fallback_sent`)로 기록한다.

---

## 5. 데이터 모델 (ERD 수준)

### 5.0 Supabase 프로젝트 공용 사용 원칙 (기술 검증 반영)

> **배경**: Supabase 무료 플랜 프로젝트 슬롯 2개(계정 한도)가 이미 다른 앱에서 사용 중이라, 우동을 위한 신규 무료 프로젝트를 만들 수 없다. 따라서 **기존 프로젝트를 그대로 공유**하며, 아래 원칙으로 다른 앱과의 충돌을 방지한다.

**네이밍 규칙(확정)**: 우동에서 새로 만드는 모든 테이블/함수/트리거는 `public` 스키마 안에 **`woodong_` 접두어**를 붙인다(별도 Postgres 스키마 분리는 하지 않음 — PostgREST 노출 스키마 설정 변경 등 다른 앱에 영향을 줄 수 있는 프로젝트 전역 설정 변경을 피하기 위함). **기존 테이블(`profiles`, `notifications`, `weekly_logs`, `organizations` 등)에는 어떠한 `ALTER`/`DROP`/`TRUNCATE`도 수행하지 않는다.**

**`profiles` 공용 재사용 여부 — 검토 결과: 가능 (읽기 재사용)**

[FACT — 실제 DB 확인] `auth.users`에 `AFTER INSERT` 트리거 `on_auth_user_created`(`handle_new_user()`)가 걸려 있어, **어느 앱을 통해 가입하든** 모든 신규 사용자에 대해 `public.profiles(id, email)` 행이 자동 생성된다(`SECURITY DEFINER`로 RLS 우회, NOT NULL 컬럼은 모두 기본값이 있어 우동 가입 시에도 실패하지 않음). 컬럼 구성:

| 컬럼                                                             | 우동에서 사용 가능 여부                                                                                                                                                                                                                                                   |
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id`, `email`, `name`, `phone_number`, `bio`                     | ✅ 읽기 재사용 가능 — 우동 화면(마이페이지, 모임 멤버 목록)에서 그대로 표시                                                                                                                                                                                               |
| `avatar_key`                                                     | ⚠️ 다른 앱의 아바타 프리셋 키(기본값 `'fox'`). 우동은 자체 이미지 업로드가 필요하면 이 컬럼을 재정의하지 않고 `woodong_groups.cover_image_object_path`처럼 우동 전용 테이블에 Storage 오브젝트 경로를 저장한다                                                            |
| `role`                                                           | ❌ 재사용 금지 — 전역 `user`/`admin`/`superadmin` 값이며 "마지막 관리자는 강등 불가" 등 다른 앱의 조직 권한 로직에 강결합된 트리거(`prevent_unauthorized_role_change`)가 걸려 있다. 우동의 총무/일반회원 구분은 `woodong_group_members.role`(모임별 값)로 완전히 분리한다 |
| `notify_on_comment` / `notify_on_mention` / `notify_on_reminder` | ❌ 재사용 금지 — 다른 앱의 주간보고 댓글/멘션 알림 전용 플래그. 우동의 채널별 알림 설정은 `woodong_notification_preferences`로 별도 관리한다                                                                                                                              |

→ **결론: `profiles`는 스키마 변경 없이 읽기 전용으로 공용 사용한다.** 우동 전용 테이블에서 사용자를 참조할 때는 `auth.users(id)`를 FK 대상으로 하고(다른 앱이 `profiles` 컬럼을 변경하더라도 우동의 참조 무결성에는 영향이 없도록), 화면에서 이름/연락처를 표시할 때만 `public.profiles`를 조인해서 읽는다.

**`notifications` 공용 재사용 여부 — 검토 결과: 불가능**

[FACT — 실제 DB 확인] 컬럼이 `recipient_id`/`actor_id`/`weekly_log_id`/`comment_id`/`period_start`로 다른 앱의 주간보고 도메인에 고정돼 있고, `notifications_protect_columns` 트리거가 `read_at` 외 모든 컬럼의 UPDATE를 차단하며, `authenticated` 역할에 대한 INSERT RLS 정책이 없어(서비스 롤 전용) 범용 알림함으로 쓸 수 없다. → **재사용하지 않고 `woodong_notifications`를 신규 생성한다.**

**적용 방침**: 아래 5.1~5.13의 모든 테이블은 `woodong_` 접두어를 붙여 `public` 스키마에 생성하고, RLS는 4.2절의 헬퍼 함수 기반 정책을 적용한다. `→`는 외래키(FK) 관계를 의미하며, `profiles`로 표시된 행은 위 원칙에 따라 **기존 공유 테이블을 그대로 참조**(신규 생성 아님)한다.

### 5.1 profiles (기존 공유 테이블 — 읽기 전용 재사용, 신규 생성 아님)

| 컬럼         | 타입      | 설명                                                                             |
| ------------ | --------- | -------------------------------------------------------------------------------- |
| id           | uuid (PK) | `auth.users.id`와 1:1, FK. 우동에서 참조 시 이 컬럼(또는 `auth.users.id`)을 사용 |
| email        | text      | 이메일(nullable — Kakao 이메일 미제공 시 null 가능)                              |
| name         | text      | 표시 이름                                                                        |
| phone_number | text      | 전화번호(nullable, 우동에서는 표시 용도로만 사용)                                |
| bio          | text      | 자기소개(nullable, 우동에서는 미사용)                                            |
| avatar_key   | text      | 다른 앱의 아바타 프리셋 키(우동은 참조만, 재정의하지 않음)                       |

### 5.2 woodong_groups (모임)

| 컬럼                    | 타입          | 설명                                                                                                                                                                            |
| ----------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| id                      | uuid (PK)     | 모임 고유 ID                                                                                                                                                                    |
| name                    | text          | 모임 이름                                                                                                                                                                       |
| description             | text          | 모임 소개                                                                                                                                                                       |
| type                    | text          | 모임 유형(동호회/계모임/스터디/취미모임 등, 확장용)                                                                                                                             |
| cover_image_object_path | text          | Storage 오브젝트 경로(`woodong-covers` 버킷, 비공개 — 조회 시 서명 URL 발급, public URL 저장 금지. `woodong_expenses.receipt_object_path`와 동일 패턴, 4.2절 Storage 정책 참고) |
| default_due_amount      | numeric(14,0) | 기본 월 회비 금액(원 단위, 소수점 없음)                                                                                                                                         |
| created_by              | uuid          | → `auth.users.id`                                                                                                                                                               |
| created_at              | timestamptz   | 생성 일시                                                                                                                                                                       |

### 5.3 woodong_group_members (모임 멤버십)

| 컬럼      | 타입        | 설명                               |
| --------- | ----------- | ---------------------------------- |
| id        | uuid (PK)   | 멤버십 고유 ID                     |
| group_id  | uuid        | → `woodong_groups.id`              |
| user_id   | uuid        | → `auth.users.id`                  |
| role      | text        | `admin`(총무) / `member`(일반회원) |
| joined_at | timestamptz | 가입 일시                          |
| status    | text        | `active` / `left`                  |

제약: `UNIQUE(group_id, user_id)`(중복 가입 방지). 마지막 남은 `admin`은 `role` 변경 또는 `status='left'` 전환이 금지되도록 기존 공유 테이블 `profiles`에 이미 걸려 있는 `prevent_unauthorized_role_change`와 유사한 트리거를 `woodong_group_members` 전용으로 별도 구현한다(5.1절, 3.2절 AC 참고).

### 5.4 woodong_group_invites (모임 초대)

| 컬럼       | 타입        | 설명                                             |
| ---------- | ----------- | ------------------------------------------------ |
| id         | uuid (PK)   | 초대 고유 ID                                     |
| group_id   | uuid        | → `woodong_groups.id`                            |
| code       | text        | 초대 코드                                        |
| created_by | uuid        | → `auth.users.id`                                |
| expires_at | timestamptz | 만료 일시                                        |
| max_uses   | integer     | 최대 사용 횟수(nullable = 무제한)                |
| used_count | integer     | 사용된 횟수(RPC로 원자적 증가, 동시성 이슈 방지) |
| is_active  | boolean     | 활성 여부(기본 `true`)                           |
| revoked_at | timestamptz | 무효화 일시(nullable)                            |

제약: `UNIQUE(code)`. 참여 가능 조건은 `is_active = true AND revoked_at IS NULL AND expires_at > now() AND (max_uses IS NULL OR used_count < max_uses)`.

### 5.5 woodong_due_cycles (회비 항목/사이클)

> 기존 설계는 "회비 항목"과 "멤버별 청구"가 하나의 `dues` 테이블에 뭉쳐 있어 항목 단위 수정이 불가능했다(기술 검증 M2). 항목(사이클)과 청구를 분리했다.

| 컬럼                   | 타입          | 설명                                               |
| ---------------------- | ------------- | -------------------------------------------------- |
| id                     | uuid (PK)     | 회비 항목 고유 ID                                  |
| group_id               | uuid          | → `woodong_groups.id`                              |
| title                  | text          | 항목명(예: `2026년 8월 정기 회비`)                 |
| period                 | text          | 대상 기간(예: `2026-08`)                           |
| amount                 | numeric(14,0) | 멤버당 청구 금액(원 단위)                          |
| due_type               | text          | `regular`(정기) / `extra`(비정기)                  |
| due_date               | date          | 납부 기한                                          |
| reminder_interval_days | integer       | 미납자 리마인드 주기(일, nullable = 리마인드 없음) |
| created_by             | uuid          | → `auth.users.id`                                  |
| created_at             | timestamptz   | 생성 일시                                          |

### 5.6 woodong_dues (멤버별 회비 청구)

| 컬럼             | 타입          | 설명                                                                                  |
| ---------------- | ------------- | ------------------------------------------------------------------------------------- |
| id               | uuid (PK)     | 청구 고유 ID                                                                          |
| due_cycle_id     | uuid          | → `woodong_due_cycles.id`                                                             |
| group_id         | uuid          | → `woodong_groups.id`(비정규화 — RLS 정책 단순화 목적)                                |
| user_id          | uuid          | → `auth.users.id`, 대상 멤버                                                          |
| amount           | numeric(14,0) | 청구 금액(생성 시점 `due_cycles.amount` 스냅샷)                                       |
| status           | text          | `unpaid` / `partial` / `paid`(연결된 `woodong_payments` 합계 기준 트리거로 자동 갱신) |
| last_reminded_at | timestamptz   | 마지막 리마인드 발송 일시(nullable)                                                   |

제약: `UNIQUE(due_cycle_id, user_id)`.

### 5.7 woodong_payments (납부 이력)

| 컬럼        | 타입          | 설명                                                   |
| ----------- | ------------- | ------------------------------------------------------ |
| id          | uuid (PK)     | 납부 이력 고유 ID                                      |
| due_id      | uuid          | → `woodong_dues.id`                                    |
| group_id    | uuid          | → `woodong_groups.id`(비정규화 — RLS 정책 단순화 목적) |
| amount      | numeric(14,0) | 실제 납부 금액                                         |
| paid_at     | timestamptz   | 납부 확인 일시                                         |
| recorded_by | uuid          | → `auth.users.id`, 확인 처리한 총무                    |
| memo        | text          | 비고(입금자명 등)                                      |

### 5.8 woodong_expenses (지출 내역, 2차 확장)

| 컬럼                | 타입          | 설명                                                                                                 |
| ------------------- | ------------- | ---------------------------------------------------------------------------------------------------- |
| id                  | uuid (PK)     | 지출 고유 ID                                                                                         |
| group_id            | uuid          | → `woodong_groups.id`                                                                                |
| category            | text          | 지출 카테고리(회식비/행사비/기타 등)                                                                 |
| amount              | numeric(14,0) | 지출 금액                                                                                            |
| receipt_object_path | text          | Storage 오브젝트 경로(`woodong-receipts` 버킷, 비공개 — 조회 시 서명 URL 발급, public URL 저장 금지) |
| paid_by             | uuid          | → `auth.users.id`, 담당자                                                                            |
| spent_at            | date          | 지출 일자                                                                                            |
| memo                | text          | 비고                                                                                                 |

### 5.9 woodong_settlements / woodong_settlement_items (정산 리포트, 2차 확장)

**woodong_settlements**

| 컬럼          | 타입          | 설명                      |
| ------------- | ------------- | ------------------------- |
| id            | uuid (PK)     | 정산 리포트 고유 ID       |
| group_id      | uuid          | → `woodong_groups.id`     |
| period_start  | date          | 정산 시작일               |
| period_end    | date          | 정산 종료일               |
| total_income  | numeric(14,0) | 총 수입(발행 시점 스냅샷) |
| total_expense | numeric(14,0) | 총 지출(발행 시점 스냅샷) |
| balance       | numeric(14,0) | 잔액                      |
| published_at  | timestamptz   | 발행 일시                 |
| published_by  | uuid          | → `auth.users.id`         |

**woodong_settlement_items** (항목별 상세 스냅샷 — 기존 설계는 총계만 저장해 "항목별 상세 내역 포함" 요구사항과 모순이었음, M4 수정)

| 컬럼          | 타입          | 설명                            |
| ------------- | ------------- | ------------------------------- |
| id            | uuid (PK)     | 항목 고유 ID                    |
| settlement_id | uuid          | → `woodong_settlements.id`      |
| item_type     | text          | `income` / `expense`            |
| category      | text          | 카테고리(회비/회식비/행사비 등) |
| amount        | numeric(14,0) | 금액                            |
| description   | text          | 설명                            |

### 5.10 woodong_votes (투표)

| 컬럼           | 타입        | 설명                                                           |
| -------------- | ----------- | -------------------------------------------------------------- |
| id             | uuid (PK)   | 투표 고유 ID                                                   |
| group_id       | uuid        | → `woodong_groups.id`                                          |
| title          | text        | 투표 제목                                                      |
| vote_type      | text        | `multiple_choice`(객관식) / `yes_no`(찬반)                     |
| allow_multiple | boolean     | 복수 선택 허용 여부(기본 `false`)                              |
| is_anonymous   | boolean     | 익명 여부                                                      |
| closes_at      | timestamptz | 마감 일시                                                      |
| status         | text        | `open` / `closed`(1차 MVP: lazy 마감 처리 시 갱신, 3.5절 참고) |
| created_by     | uuid        | → `auth.users.id`                                              |
| created_at     | timestamptz | 생성 일시                                                      |

### 5.11 woodong_vote_options / woodong_vote_responses (투표 선택지/응답)

**woodong_vote_options**

| 컬럼       | 타입      | 설명                 |
| ---------- | --------- | -------------------- |
| id         | uuid (PK) | 선택지 고유 ID       |
| vote_id    | uuid      | → `woodong_votes.id` |
| label      | text      | 선택지 텍스트        |
| sort_order | integer   | 표시 순서            |

**woodong_vote_responses**

| 컬럼         | 타입        | 설명                                                                                               |
| ------------ | ----------- | -------------------------------------------------------------------------------------------------- |
| id           | uuid (PK)   | 응답 고유 ID                                                                                       |
| vote_id      | uuid        | → `woodong_votes.id`                                                                               |
| option_id    | uuid        | → `woodong_vote_options.id`                                                                        |
| user_id      | uuid        | → `auth.users.id`(익명 투표에서도 중복 투표 방지를 위해 필수 저장 — RLS로 노출만 제한, 4.2절 참고) |
| responded_at | timestamptz | 응답 일시                                                                                          |

제약: `allow_multiple = false`인 투표는 `UNIQUE(vote_id, user_id)`, `allow_multiple = true`인 투표는 `UNIQUE(vote_id, user_id, option_id)`로 애플리케이션/트리거 단에서 분기 검증한다(단일 CHECK로 표현 불가하므로 `woodong_votes.allow_multiple`을 조회하는 `BEFORE INSERT` 트리거로 검증).

### 5.12 woodong_announcements (공지사항 — 신규 추가)

> 기존 설계는 3.3/6.3의 "공지 등록/목록/작성" 기능과 `woodong_notifications.related_id`("관련 리소스: 공지")를 전제로 했으나 공지 본문을 저장할 테이블이 ERD에 없었다(기술 검증 C4). 신규 추가.

| 컬럼       | 타입        | 설명                  |
| ---------- | ----------- | --------------------- |
| id         | uuid (PK)   | 공지 고유 ID          |
| group_id   | uuid        | → `woodong_groups.id` |
| title      | text        | 공지 제목             |
| body       | text        | 공지 본문             |
| created_by | uuid        | → `auth.users.id`     |
| created_at | timestamptz | 작성 일시             |
| updated_at | timestamptz | 수정 일시             |

### 5.13 woodong_notifications / woodong_notification_preferences (알림)

**woodong_notifications** (알림 발송 이력)

| 컬럼         | 타입        | 설명                                                                                                  |
| ------------ | ----------- | ----------------------------------------------------------------------------------------------------- |
| id           | uuid (PK)   | 알림 고유 ID                                                                                          |
| group_id     | uuid        | → `woodong_groups.id` (nullable, 시스템 알림은 null 가능)                                             |
| user_id      | uuid        | → `auth.users.id`, 수신자                                                                             |
| type         | text        | `notice`(공지) / `due_reminder`(회비 리마인드) / `vote_start` / `vote_close` / `settlement_published` |
| related_type | text        | `announcement` / `vote` / `settlement` / `due`(다형 참조 구분, nullable)                              |
| related_id   | uuid        | `related_type`이 가리키는 리소스 ID(nullable)                                                         |
| channel      | text        | `web_push` / `in_app`                                                                                 |
| status       | text        | `pending` / `sent` / `failed` / `fallback_sent`                                                       |
| title        | text        | 알림 제목                                                                                             |
| body         | text        | 알림 본문                                                                                             |
| read_at      | timestamptz | 읽음 처리 일시(nullable — 알림센터 읽음/안읽음, 8장 KPI "알림 클릭률" 산출 기반)                      |
| clicked_at   | timestamptz | 클릭 일시(nullable)                                                                                   |
| created_at   | timestamptz | 생성 일시                                                                                             |

**woodong_notification_preferences** (알림 채널 설정)

| 컬럼        | 타입        | 설명                                                                                                                                                                           |
| ----------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| id          | uuid (PK)   | 설정 고유 ID                                                                                                                                                                   |
| user_id     | uuid        | → `auth.users.id`                                                                                                                                                              |
| channel     | text        | `web_push` / `in_app`                                                                                                                                                          |
| enabled     | boolean     | 활성화 여부                                                                                                                                                                    |
| destination | text        | 채널별 수신처(`web_push`: 브라우저 Push 구독 정보(endpoint/keys)를 JSON으로 자동 저장 — 사용자가 알림을 허용하면 자동 등록되며 직접 입력하지 않음, `in_app`: 미사용, nullable) |
| updated_at  | timestamptz | 수정 일시                                                                                                                                                                      |

제약: `UNIQUE(user_id, channel)`.

> **참고**: 실제 마이그레이션(`mcp__supabase__apply_migration`) 적용은 이 PRD 승인 이후 별도 단계로 진행하며, 적용 전 반드시 이 문서의 최신 버전 기준으로 재확인한다. 스키마 변경 후에는 `mcp__supabase__generate_typescript_types`로 `lib/supabase/database.types.ts`를 재생성한다(다른 앱의 테이블 타입도 함께 재생성되지만 이는 타입 정의 파일 갱신일 뿐 DB에는 영향이 없다).

---

## 6. 화면 목록 (IA/사이트맵)

기존 코드베이스의 `app/` 구조(루트에 `src/` 없음, `app/auth/*`는 인증, `app/protected/*`는 보호 영역)와 `proxy.ts`의 공개 페이지 allow-list 패턴을 그대로 따른다.

### 6.1 인증 플로우 (공개, `app/auth/*` — 기존 구조 재사용)

| 화면     | 목적                                                                    |
| -------- | ----------------------------------------------------------------------- |
| 로그인   | 이메일/비밀번호 로그인 + Google/Kakao 소셜 로그인 버튼                  |
| 회원가입 | 이메일/비밀번호 가입(가입 즉시 세션 발급, 이메일 확인 절차 없음 — v1.3) |

> 비밀번호 찾기/재설정(이메일 기반)은 1차 MVP에서 제외한다(v1.4). 대신 로그인된 사용자를 위한 "비밀번호 변경" 기능을 `app/protected/me`(마이페이지)에 제공한다 — 6.4절 참고.

### 6.2 공개 페이지 (비로그인 접근 가능, `proxy.ts` allow-list 등록 필요)

| 화면            | 목적                                                                 |
| --------------- | -------------------------------------------------------------------- |
| 메인(랜딩)      | 서비스 소개, 기술 스택 인포그래픽, 로그인 상태별 CTA (3.1)           |
| 아이콘 검색     | lucide 아이콘 검색 — **기존 `/icons` 재사용** (3.8)                  |
| 컴포넌트 갤러리 | shadcn/ui 컴포넌트 데모 — **기존 `/gallery` 재사용** (3.8)           |
| 초대 참여       | 초대 코드/링크로 모임 참여(로그인 필요 시 로그인 유도 후 복귀) (3.2) |

### 6.3 보호 페이지 — 모임 관리 (`app/protected/groups/*`, 로그인 필요)

| 화면               | 목적                                                                  | 관련 기능          |
| ------------------ | --------------------------------------------------------------------- | ------------------ |
| 모임 목록          | 내가 속한 모임 카드 리스트, 새 모임 만들기 진입점                     | 3.2                |
| 모임 생성          | 모임 이름/설명/기본 회비 입력                                         | 3.2                |
| 모임 상세(홈)      | 모임 요약(공지, 회비 납부율, 진행 중 투표) 대시보드                   | 3.2, 3.3, 3.4, 3.5 |
| 모임 설정          | 모임 정보 수정, 멤버 목록/역할 관리, 초대 링크 발급, 모임 삭제        | 3.2                |
| 공지사항 목록/작성 | 공지 목록 확인, 신규 공지 작성 및 발송                                | 3.3                |
| 회비 대시보드      | 멤버별 납부 현황(진행률 바), 회비 항목 생성, 리마인드 주기 설정       | 3.4                |
| 지출 등록/내역     | 지출 목록 확인, 신규 지출 등록(영수증 첨부)                           | 3.4                |
| 정산 리포트        | 기간별 정산 리포트 목록, 신규 리포트 생성/발행, 상세 조회 및 다운로드 | 3.4                |
| 투표 목록          | 진행 중/마감된 투표 목록                                              | 3.5                |
| 투표 생성          | 제목/선택지/마감일/익명여부 설정                                      | 3.5                |
| 투표 상세/참여     | 선택지 선택 후 투표, 마감 전 상태 확인                                | 3.5                |
| 투표 결과          | 마감 후 집계 결과(익명/실명에 따라 다른 표시)                         | 3.5                |

### 6.4 보호 페이지 — 공통 (로그인 필요)

| 화면       | 목적                                                                                                                                               | 관련 기능 |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| 알림센터   | 채널 무관 전체 알림 이력 확인, 읽음 처리                                                                                                           | 3.3       |
| 마이페이지 | 프로필 수정, **비밀번호 변경(이메일 없이 화면 내에서 현재 비밀번호 확인 후 처리, v1.4)**, 알림 채널 설정(웹 푸시/앱내 on/off), 소셜 계정 연동 관리 | 3.3, 3.6  |

---

## 7. MVP 범위 정의

우선순위 기준: **"총무의 반복 업무 자동화"에 가장 크게 기여하는 기능 순.**

> **재조정 배경(기술 검증 반영)**: 최초 범위(테이블 12개, RLS 정책 30~~40개, 라우트 24개 기준)의 1인 개발 공수를 재추정한 결과 163~~212h로, 4주(약 160h) 상한 대비 최대 30% 초과하는 것으로 나타났다. DB 환경 정비(5.0절) 공수까지 더하면 초과폭이 더 커진다. **기간을 늘리는 대신 범위를 축소해 4주 내 완결 가능한 버전을 우선 출시**하고, 공수가 큰 항목(회비 지출/정산 발행, 실시간 스케줄러, Naver/Apple 로그인)을 2차로 이동한다.

### 7.1 1차 MVP (4주 내 개발 가능한 수준)

| 우선순위 | 기능                              | 포함 범위                                                                                                                                                                                                                                                             |
| -------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0        | Supabase 프로젝트 정비 (5.0, 4.2) | 공유 프로젝트에 `woodong_*` 테이블 생성, RLS 헬퍼 함수(`woodong_is_group_member`/`woodong_is_group_admin`) 구현, Storage 비공개 버킷 2개 생성, `database.types.ts` 재생성. **다른 기능 개발의 선행 작업**                                                             |
| 1        | 인증 (3.6)                        | 이메일 회원가입/로그인 + Google 소셜 로그인. Kakao는 이메일 미제공 예외 처리(3.6.1)까지 포함해 1차에 구현. **Naver는 1차 범위에서 완전히 제외**(스파이크 후 2차 여부 결정)                                                                                            |
| 2        | 모임 관리 (3.2)                   | 모임 생성/수정/삭제, 초대 코드 기반 참여(무효화/재발급 포함), 역할(총무/일반회원) 구분, 마지막 총무 보호 로직                                                                                                                                                         |
| 3        | 회비 현황 관리 (3.4-a)            | 회비 항목(`woodong_due_cycles`) 생성, 납부 상태 수동 변경, 납부 현황 대시보드. **지출 등록·정산 리포트 발행(3.4-b)은 2차로 이동**                                                                                                                                     |
| 4        | 모임 알림 (3.3) — 앱 내 알림만    | 공지 작성(`woodong_announcements`) 시 앱 내 알림센터 발송, 읽음/클릭 처리. 회비 리마인드·투표 마감 알림은 **조회 시점 lazy 처리**로 구현(실시간 pg_cron 스케줄러는 2차). 외부 채널(웹 푸시)은 2차로 연기(v1.5, 카카오톡 알림톡/Slack/이메일은 로드맵에서 완전히 제외) |
| 5        | 투표 관리 (3.5)                   | 투표 생성(객관식/찬반, 복수선택 옵션), 참여, **lazy 마감**(조회 시점 자동 마감 + 총무 수동 조기마감), 앱 내 알림으로 결과 통지                                                                                                                                        |
| 6        | 모임 메인 페이지 (3.1)            | 서비스 소개 + 로그인 상태별 CTA (기술 스택 인포그래픽은 간소화 버전)                                                                                                                                                                                                  |
| 7        | 디자인 시스템 (3.7)               | 모바일 반응형, shadcn/ui 기반 기본 컴포넌트, recharts 기반 진행률 바                                                                                                                                                                                                  |
| 8        | 푸터 특수 요구사항 (3.8)          | 기존 `/icons`, `/gallery` 링크만 추가 (신규 개발 없음)                                                                                                                                                                                                                |

### 7.2 2차 확장 (MVP 이후)

- 회비 지출 등록(영수증 첨부, Storage 서명 URL) 및 정산 리포트 발행/PDF 다운로드(3.4-b)
- 실시간 배치 스케줄러(Supabase Cron/pg_cron) 도입 — 회비 리마인드, 투표 마감/집계, 정산 발행 알림을 lazy 처리에서 실시간 발송으로 전환
- 외부 알림 채널 연동: 웹 푸시(Web Push, PWA 기반 — VAPID/Service Worker) — 재시도/폴백 정책 포함
- Naver 소셜 로그인(스파이크 결과에 따라 결정) 및 Apple 소셜 로그인(iOS 앱 래핑 계획 구체화 시)
- 계정 자동 연결 시 이메일 알림 발송(3.6.2 정책 5번)
- Supabase Storage 플랜 업그레이드 시 서버 사이드 이미지 변환 전환
- 모임 유형 다양화(스터디, 취미모임 템플릿)에 따른 UI 커스터마이징
- 알림센터 고급 필터링(채널별/유형별)
- 총무 교체 대비 정산 데이터 CSV 내보내기

---

## 8. 성공 지표 (KPI)

| 지표                                      | 정의                                                                   | 목표(MVP 출시 후 8주 기준) |
| ----------------------------------------- | ---------------------------------------------------------------------- | -------------------------- |
| 주간 활성 모임 수(WAG)                    | 주간 1회 이상 로그인 멤버가 있는 모임 수                               | 20개 이상                  |
| 회비 납부율 개선폭                        | 서비스 도입 전(자체 신고) 대비 평균 납부율 상승폭                      | +15%p 이상                 |
| 총무 1인당 정산 소요시간 감소             | 정산 리포트 생성까지 걸리는 총무 작업 시간(설문 기반, 도입 전/후 비교) | 50% 이상 감소              |
| 알림 클릭률                               | 발송된 알림 중 클릭/열람된 비율                                        | 40% 이상                   |
| 투표 참여율                               | 모임 멤버 대비 투표 응답 비율                                          | 60% 이상                   |
| 신규 모임 생성 → 첫 회비 항목 생성 전환율 | 모임 생성 후 7일 내 회비 항목을 1개 이상 등록한 비율                   | 70% 이상                   |

---

## 9. 리스크 및 결정 필요 사항

| 구분                                                                     | 내용                                                                                                                                                                                                                                                 | 대응/결정 필요 사항                                                                                                                                                                                                                                                            |
| ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Supabase 프로젝트 공유** (신규, Critical→해결됨)                       | 무료 플랜 슬롯 2개가 이미 다른 앱에 사용 중이라 우동 전용 프로젝트를 만들 수 없음                                                                                                                                                                    | **해결**: 5.0절 원칙대로 `woodong_` 접두어 테이블 격리 + 기존 테이블 무변경으로 동일 프로젝트 공유. 잔여 리스크는 DB 용량(Free 500MB)·커넥션 풀을 다른 앱과 공유한다는 점 — 사용량 모니터링 후 필요 시 유료 플랜 전환 또는 신규 프로젝트 확보 시점에 `woodong_*` 테이블만 이관 |
| **Naver OAuth 미지원** (수정됨)                                          | [FACT] Supabase Auth는 Naver를 네이티브 provider로 지원하지 않음                                                                                                                                                                                     | 1차 MVP 범위에서 제외(3.6.1). 2차 착수 전 Custom OAuth/자체 구현 적합성을 1~2일 스파이크로 검증 후 진행 여부 결정                                                                                                                                                              |
| **계정 자동 연결 정책** (재작성 완료)                                    | [FACT] Supabase는 verified 이메일 기준 자동 identity 연결이 기본 동작이며 끌 수 없음                                                                                                                                                                 | **결정 완료**: "자동 연결 수용 + 사후 고지" 정책으로 3.6.2절에 확정. 추가 결정 필요 사항: 자동 연결 발생 시 이메일 알림까지 1차에 포함할지(현재는 2차로 분류)                                                                                                                  |
| **Kakao Biz App 요건이 로그인에도 적용됨** (신규)                        | [FACT] `account_email` 동의항목은 Biz App(사업자 정보 등록) 완료 앱에서만 제공됨. 개인 개발자 앱은 Kakao 로그인 시 이메일을 받지 못할 수 있음(알림 발송 채널로서의 카카오톡 알림톡은 v1.5에서 로드맵 제외, 본 리스크는 **로그인** 플로우에만 적용됨) | Supabase 프로젝트에서 "Allow users without an email" 활성화를 1차 착수 시점에 미리 결정. Biz App 등록(사업자 정보 필요) 여부와 일정을 조기에 확인 — 등록 전이면 이메일 없는 Kakao 계정을 정상 플로우로 처리(3.6.2절 예외 케이스)                                               |
| **웹 푸시(Web Push) iOS 제약** (v1.5, 카카오톡 알림톡/Slack/이메일 대체) | [FACT] iOS Safari는 iOS 16.4부터 웹 푸시를 지원하지만, **홈 화면에 추가(PWA 설치)한 경우에만** 동작한다. 브라우저 탭으로만 접속하는 iOS 사용자는 푸시를 받을 수 없음                                                                                 | 1차 MVP는 앱 내 알림으로 대체. 2차 확장(웹 푸시) 도입 시 iOS 사용자에게 "홈 화면에 추가" 온보딩 유도 UI를 함께 제공하고, 미설치 사용자는 앱 내 알림으로 폴백                                                                                                                   |
| **Storage 플랜 제약** (신규)                                             | [FACT] Supabase Storage의 서버 사이드 이미지 변환은 Pro 플랜 이상 전용. 본 프로젝트는 Free 플랜을 공유 중                                                                                                                                            | 1차 MVP는 클라이언트 사이드 리사이즈로 대체(4.1절). Free 플랜의 "7일 미사용 시 프로젝트 일시정지" 정책도 함께 고려해, 데모/QA 기간 중 정기적으로 프로젝트에 접근하도록 운영                                                                                                    |
| **개인정보보호법(PIPA) 대응** (신규)                                     | 회비 납부 이력(실명 + 금액) + 전화번호 조합은 개인정보 수집·이용 동의 및 처리방침이 필요한 개인정보에 해당할 가능성이 높음                                                                                                                           | 서비스 출시 전 개인정보 처리방침 초안 작성 및 회원가입 시 동의 절차 추가 여부 결정(법률 자문 권장)                                                                                                                                                                             |
| **총무 단일 실패점** (신규)                                              | 모임의 마지막 총무가 탈퇴하거나 역할을 변경하면 해당 모임이 관리 불능 상태가 될 수 있음                                                                                                                                                              | 5.3절 제약 및 3.2절 AC로 "마지막 총무는 역할 변경/탈퇴 불가" 방어 로직을 1차 MVP에 포함하기로 결정 완료                                                                                                                                                                        |
| **정산 데이터 이관 부재** (신규)                                         | 총무 교체 시 정산 데이터를 새 총무에게 인계할 CSV 내보내기 등의 수단이 없음                                                                                                                                                                          | 1차 MVP 범위에서는 제외하고 2차 확장 항목으로 등록(7.2절)                                                                                                                                                                                                                      |
| **금융 정보 취급 법적 고지**                                             | 회비/정산은 실제 계좌이체를 서비스 내에서 직접 처리하지 않고 "납부 확인 기록"만 다루므로 전자금융업 등록 대상은 아닐 가능성이 높으나, 법률 자문을 통한 최종 확인 필요                                                                                | 서비스 내 결제/PG 연동은 MVP 범위에서 명시적으로 제외. 이용약관에 "회비 송금은 사용자 간 직접 이체이며 본 서비스는 확인·기록만 제공함"을 명시할지 결정 필요                                                                                                                    |
| 초대 코드 보안                                                           | 초대 코드가 유출될 경우 원치 않는 인원이 모임에 가입할 위험                                                                                                                                                                                          | 만료 기간 및 최대 사용 횟수(`woodong_group_invites.max_uses`) 설정 필수화, 총무가 언제든 코드 재발급(기존 코드 `is_active=false` 무효화, 5.4절) 가능하도록 설계 완료                                                                                                           |
| 정산 데이터 정확성                                                       | 회비 수입과 지출이 수동 입력에 의존하므로 총무의 입력 누락/오류 시 정산 리포트 신뢰도 저하                                                                                                                                                           | 정산 리포트(2차 확장, 3.4-b) 발행 전 "검토 단계"(초안 → 발행) 도입 여부 결정 필요                                                                                                                                                                                              |
| 알림 발송 비용                                                           | (v1.5 갱신) 웹 푸시는 발송 건당 과금이 없어 카카오톡 알림톡/이메일 대비 비용 리스크가 크게 낮아짐                                                                                                                                                    | 별도 비용 시뮬레이션은 불필요. 다만 브라우저 푸시 서비스(FCM/APNs 경유) 구독의 만료/무효화 정리 로직은 필요                                                                                                                                                                    |
| **계정 잠김 시 자기 복구 수단 없음** (신규, v1.4)                        | 비밀번호 재설정을 이메일 없이 "로그인 상태의 화면 내 변경"으로만 제공하기로 결정(9장 표 상단, SMTP 미설정 제약). 비밀번호를 잊고 동시에 소셜 로그인도 연동하지 않은 사용자는 스스로 계정을 복구할 방법이 없음                                        | 1차 MVP는 수용 가능한 리스크로 판단(Supabase 대시보드에서 관리자가 수동으로 비밀번호를 재설정해줄 수 있음). 커스텀 SMTP(Resend 등) 연동 시 이메일 기반 복구 플로우 재도입 여부 결정 필요                                                                                       |

---

## 부록: 정합성 참고 사항 (CLAUDE.md 기준)

- 신규 페이지 추가 시 `lib/supabase/proxy.ts`의 `updateSession()` 공개 페이지 allow-list에 반드시 등록해야 한다(6.2절 공개 페이지 대상).
- `getClaims()`, `<Suspense>` 경계, 얇은 `Page` + `XxxContent` 패턴은 모든 신규 페이지(특히 회비 대시보드, 투표 상세 등 request-time API를 사용하는 서버 컴포넌트)에 동일하게 적용한다.
- 신규 UI 문자열은 `lib/i18n/dictionaries/{ko,en,ja,zh}.ts` 4개 파일과 `lib/i18n/dictionaries/types.ts`의 `Dictionary` 타입에 함께 반영해야 한다. **(정정)** `Dictionary`는 strict 인터페이스이고 pre-commit 훅이 `tsc --noEmit`을 강제하므로, en/ja/zh 키를 아예 비워두면 커밋 자체가 실패한다. 1차 MVP는 한국어(`ko`) 문구를 우선 확정하되, en/ja/zh 3개 파일에도 최소한 **동일 한국어 문자열을 스텁으로 복사**해 타입을 충족시키고, 실제 번역은 2차 확장에서 순차 진행한다.
- 스키마 변경(본 PRD 5장의 `woodong_*` 테이블 생성) 후에는 `mcp__supabase__generate_typescript_types`로 `lib/supabase/database.types.ts`를 재생성해야 한다. 이 프로젝트는 다른 앱과 공유 중이므로 재생성된 파일에는 `weekly_logs`, `organizations` 등 다른 앱의 테이블 타입도 함께 포함되지만, 이는 타입 정의 파일 갱신일 뿐 실제 DB에는 영향이 없다.
- 색상 테마(코럴/앰버)를 적용할 때는 `app/globals.css`의 `:root`/`.dark`와 `tailwind.config.ts`의 `theme.extend.colors`를 함께 수정한다(v3 HSL 변수 방식 유지).
- **(신규)** 저장소에는 react-hook-form + zod + Server Actions 기반 폼 가이드(`docs/guides/forms-react-hook-form.md`)와, `login-form.tsx` 등 기존 인증 폼의 "Client Component에서 `supabase.auth.*` 직접 호출" 패턴이 공존한다. 우동의 신규 폼(모임 생성, 회비 항목 생성, 지출 등록, 투표 생성 등)은 **가이드 문서의 react-hook-form + zod + Server Actions 패턴**을 기본으로 채택하고, `cacheComponents: true` 환경에서 뮤테이션 후 데이터 갱신은 `revalidatePath`/`revalidateTag`로 처리한다(인증 폼만 기존 Client Component 직접 호출 패턴을 그대로 유지).
- **(신규)** `app/protected/page.tsx`의 서버 컴포넌트 이중 방어 패턴과 별개로, 로그인 리다이렉트 후 원래 경로로 복귀하는 기능은 현재 미구현 상태다(`proxy.ts`가 `next` 파라미터 없이 `/auth/login`으로 보내고, `login-form.tsx`가 `/protected`로 하드코딩 리다이렉트). 3.6.2절 AC에 따라 이 3개 지점(`proxy.ts`, `login-form.tsx`, 콜백 라우트)을 함께 수정하는 작업을 1차 MVP 개발 태스크에 포함한다.
- **(신규)** 현재 `/avatars`, `/charts`, `/about`, `/tech-stack`은 스타터킷 데모 페이지로 `proxy.ts` allow-list에 남아 있다. 본 PRD 3.8/6.2절은 `/icons`, `/gallery`만 우동 서비스에 재사용하기로 했으므로, 나머지 4개 페이지를 그대로 존치할지(내부 문서용으로 유지) 제거할지는 개발 착수 전 별도 결정이 필요하다(방치 시 사용자에게 완성되지 않은 스타터킷 잔재로 노출될 위험).
