...
| 주차       | 주요 Phase                              | 산출물                                                                           |
| ---------- | --------------------------------------- | -------------------------------------------------------------------------------- |
| **Week 1** | Phase 0 + Phase 1                       | `udong_*` 스키마·RLS·Storage 준비 완료, 전체 라우트 골격과 타입/폼 아키텍처 확정 |
...
- **Task 013: 인포그래픽/시각화 컴포넌트 및 반응형 검증**
...
- **Task 014: 모임 메인(랜딩) 페이지 구현 (PRD 우선순위 6)**
  - 비로그인: 서비스 소개 섹션 + 핵심 기능 3가지 요약 + 기술 스택 인포그래픽(**1차는 간소화 버전**) + "무료로 시작하기" CTA
...
- **Task 019: 모임 CRUD 구현**
  - 모임 생성 Server Action: 생성자를 **자동으로 `admin`(총무)로 `udong_group_members`에 등록**하고 상세 페이지로 이동
  - 필수 항목(모임 이름) 미입력 시 유효성 검사 에러 표시 및 요청 미전송
  - 모임 정보 수정(이름/설명/대표 이미지/`default_due_amount`/`type`) — 대표 이미지는 `udong-covers` 비공개 버킷 업로드 + 클라이언트 리사이즈 + 서명 URL 조회
...
- **Task 022: 회비 항목(`udong_due_cycles`) 생성 및 청구 팬아웃**
  - 회비 항목 생성 폼: 제목(`title`), 대상 기간(`period`), 금액(`amount`), 납부 기한(`due_date`), 리마인드 주기(`reminder_interval_days`), `due_type`(`regular`/`extra`)
  - 생성 시 **해당 시점의 모든 활성 멤버에 대해 `udong_dues` 레코드를 `unpaid` 상태로 자동 생성**(멤버당 1건, `UNIQUE(due_cycle_id, user_id)`로 중복 방지)
  - `udong_dues.amount`는 생성 시점 `udong_due_cycles.amount` 스냅샷으로 저장, `group_id`는 RLS 단순화를 위해 비정규화 저장
...
- **Task 023: 납부 이력 기록 및 상태 자동 갱신**
  - 총무가 특정 멤버 상태를 "납부완료/부분납부"로 변경 시 `udong_payments`에 이력 기록(`amount`, `paid_at`, `recorded_by`, `memo`)
  - `udong_dues.status`는 연결된 `udong_payments.amount` **합계와 `udong_dues.amount` 비교로 `paid`/`partial` 자동 갱신**(Task 003 트리거) — 애플리케이션에서 직접 status를 쓰지 않음
  - 총무 외 사용자의 상태 변경 시도는 `udong_is_group_admin()` 정책으로 차단
...
- **Task 024-1: 회비 관리 통합 테스트 (Playwright MCP)**
  - **## 테스트 체크리스트**
    ...
    - 납부 상태 변경 → `udong_payments` 기록 → `status` 자동 갱신 → 대시보드 즉시 반영
...
- **Task 025: 공지사항(`udong_announcements`) CRUD 및 알림 팬아웃**
  - 공지 목록/작성/수정 UI 및 Server Action(작성·수정 권한은 `udong_is_group_admin()`)
  - 발송 시 각 멤버의 `udong_notification_preferences`에서 **`in_app`이 활성화된 대상 기준으로 `udong_notifications` 레코드 생성**(1차는 `in_app`만 발송)
...
- **Task 027: 알림 채널 설정 (마이페이지)**
  - `udong_notification_preferences` 기반 채널별 on/off UI(`kakao`/`slack`/`email`/`in_app`) — **1차에서 실제 발송은 `in_app`만**, 나머지는 설정만 저장하고 "2차 지원 예정" 안내 표기
...
- **Task 029: 투표 생성 및 참여 구현**
  ...
  - 생성 시 `udong_votes` + `udong_vote_options` 생성 및 **멤버 전원에게 "새 투표 시작" 알림(`type='vote_start'`)** 기록
...
- **Task 035: 회비 지출 등록 구현 (3.4-b)**
  - `udong_expenses` 테이블 생성(1차에서 미생성한 경우) 및 RLS 정책 적용
  - 지출 등록 폼(카테고리, 금액, 담당자, 지출 일자, 비고) — 필수 항목(금액, 카테고리) 누락 시 유효성 에러
  - 영수증 이미지를 `udong-receipts` **비공개 버킷**에 업로드하고 `receipt_object_path`만 저장(public URL 저장 금지), 조회는 `createSignedUrl()`
  - 지출 목록/상세 UI 및 회비 대시보드의 "잔액" 표기 활성화

- **Task 036: 정산 리포트 발행 및 다운로드 구현 (3.4-b)**
  - `udong_settlements` / `udong_settlement_items` 생성 및 발행 시점 **스냅샷** 저장(총 수입/총 지출/잔액 + 항목별 상세)
...
- **Task 038: 외부 알림 채널 연동**
  ...
  - **재시도/폴백 정책**(PRD 4.4): 최대 3회, 지수 백오프(1분 → 5분 → 15분), 최종 실패 시 앱 내 알림으로 폴백하고 `udong_notifications.status`에 `pending`/`sent`/`failed`/`fallback_sent` 기록
...
| 리스크                                                                         | 상태                                                                                  | 로드맵 반영 위치                                     |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| Supabase 프로젝트 공유(무료 슬롯 부족)                                         | 해결 — `udong_` 접두어 격리 + 기존 테이블 무변경                                      | Task 001, 002 / 잔여: 용량·커넥션 모니터링(Task 034) |
