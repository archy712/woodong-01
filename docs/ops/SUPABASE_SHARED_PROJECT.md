...
| **OAuth Provider**                           | **Google, Kakao 등록 완료**  | PRD 우선순위: Google/Kakao는 1차 필수, Naver는 1차 완전 제외(Task 039 스파이크에서 **Custom OIDC provider로 연동 가능**을 확인 — 조건부 GO, 실구현은 운영자 조치 후 Task 040. `docs/ops/NAVER_LOGIN_SPIKE.md`), Facebook 보류. Task 016에서 provider별 Client ID/Secret과 콜백 URL(`https://<project-ref>.supabase.co/auth/v1/callback`) 등록 완료.                                                                                                     |
...
> 🔌 **Custom OAuth/OIDC provider**: 이 프로젝트의 GoTrue는 `auth.custom_oauth_providers`를 지원하며 현재 등록된 provider는 0건이다(2026-08-30, Task 039 확인). **무료 플랜 쿼터는 프로젝트 전체 3개**이고 공유 프로젝트이므로, 등록·삭제 시 `identifier`를 반드시 확인해 남의 앱 provider를 건드리지 않는다.
