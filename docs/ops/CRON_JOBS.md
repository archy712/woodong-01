...
- **배치가 만드는 알림은 한국어 고정이다.** 알림 제목·본문은 생성 시점에 DB에 문자열로 저장되는데, pg_cron에는 사용자 로케일이 없어 호출부가 i18n 문구를 넘겨줄 수 없다. lazy 처리 시절에는 화면의 로케일을 그대로 넘겼기 때문에 en/ja/zh 사용자는 번역된 리마인드를 받았다 — 이 부분은 전환의 대가다. 해결하려면 사용자별 로케일을 저장하거나(예: `woodong_profiles.locale`), 알림을 `type` + 관련 엔티티로부터 **읽는 시점에** 조립하도록 모델을 바꿔야 한다. Task 040(i18n 고도화)에서 다룬다.
